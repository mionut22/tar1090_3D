...
function fetchDone(data) {
    try {
        ...
        const isVisible = document.getElementById('iframeModal').checkVisibility(); // style.display != 'block';
        if (isVisible) {
            if (theData == data) {
                console.log("data already sent to iframe, skipping");
                return;
            }
            theData = data;
            delete theData.buffer;
            theData.resultCount = data.aircraft.length;
            theData.callsign = SelectedPlane.icao;
            const iframe = document.getElementById('modalIframe');
            const json = JSON.stringify(theData);
...
function fetchData(options) {
    ...
    if (uuid != null) {
        ...
        for (let i in filterUuid) {
            let url = 'http://tilegen.ami:8081/re-api/?' + (binCraft ? 'binCraft' : 'json');
...
function setIntervalTimers() {
    ...
    timers.everySecond = setInterval(everySecond, 400);
...
function webglAddLayer() {
    ...
    try {
        ...
        let shStyle = {};
        for (var key in glStyle) {
            shStyle[key] = glStyle[key];
        }
        shStyle["icon-color"] = ['color', 0, 0, 0, 0.7];
        shStyle["icon-displacement"] = [3, 3];
        shStyle["icon-opacity"] = 0.5;
        ...
        shadowLayer = new ol.layer.WebGLPoints({
            name: 'shadowLayer',
            type: 'overlay',
            title: 'Aircraft Shadows',
            source: webglFeatures,
            declutter: false,
            zIndex: 199,
            visible: true,
        ...
        if (loStore['shadowTested'] != 'true' && !shadowLayer.getRenderer()) {
            return false;
        }
        ...
        loStore['shadowTested'] = 'true';
        success = true;
    } catch (error) {
        try {
            layers.remove(webglLayer);
            layers.remove(shadowLayer);
        } catch (error) {
            console.error(error);
        }
        console.error(error);
...
function ol_map_init() {
    ...
    OLMap.on('pointermove', function (evt) {
        ...
        if (hoveredFeature !== feature) {
            // Check if the previous feature exists and has the .set method
            if (hoveredFeature && typeof hoveredFeature.set === 'function') {
                hoveredFeature.set('hovered', false);
            }
            // Check if the current feature exists and has the .set method
    ...
    OLMap.on('movestart', function (event) {
        if (webgl) {
            if (TrackedAircraftPositions > webglIconMapMoveOpacityCrowdedThreshold) {
                ...
                shadowLayer.setOpacity(webglIconMapMoveOpacityCrowded)
            } else {
                webglLayer.setOpacity(webglIconMapMoveOpacity)
                shadowLayer.setOpacity(webglIconMapMoveOpacity)
            }
        }
    });
    ...
    OLMap.on('moveend', function (event) {
        ...
        webgl && shadowLayer.setOpacity(webglIconOpacity);
    });
...
function refreshSelected() {
    ...
    if (!selected) {
        ...
        if (map3dButton) {
            map3dButton.style.display = 'none';
            delete map3dButton.dataset.url;
        }
        selIcao = null;
    ...
    if (useRouteAPI) {
        if (selected.routeString) {
            jQuery('#selected_route').html(selected.routeString);
...
function refreshHighlighted() {
    ...
    if (useRouteAPI) {
        if (highlighted.routeString) {
            jQuery('#highlighted_route').html(highlighted.routeString);
        } else {
...
function setLineWidth() {
    ...
    bgFill = new ol.style.Stroke({ color: 'rgba(0,0,0,0.1' });
}
...
function initModal3dLeftResize() {
    if (window.__modal3dResizeInit) {
        return;
    }
    window.__modal3dResizeInit = true;

    const handle = document.getElementById('modal3dResizeLeft');
    const modalContent = document.querySelector('.modal3d-content');
    const modalIframe = document.getElementById('modalIframe');
    const mapContainer = document.getElementById('map_container');
    const layoutContainer = document.getElementById('layout_container');
    if (!handle || !modalContent) {
        return;
    }
    ...
    const minLeft = 120;
    let resizing = false;
    let fixedRightEdge = 0;
    let mapSplitRefreshTimer = null;

    const notifyModal3dResize = () => {
        if (!modalIframe || !modalIframe.contentWindow) {
            return;
        }
        modalIframe.contentWindow.postMessage({ type: 'tar1090-3d-resize' }, '*');
    };

    const refreshSplitMapLayout = () => {
        if (layoutContainer) {
            void layoutContainer.offsetWidth;
        }
        if (mapContainer) {
        ...
        updateMapSize();
        if (OLMap) {
            if (typeof OLMap.renderSync === 'function') {
                OLMap.renderSync();
            } else {
                OLMap.render();
            }
        ...
        if (mapSplitRefreshTimer) {
            clearTimeout(mapSplitRefreshTimer);
        }
        ...
        requestAnimationFrame(() => {
            ...
            notifyModal3dResize();
            mapSplitRefreshTimer = setTimeout(() => {
                if (layoutContainer) {
                    void layoutContainer.offsetWidth;
                }
                if (mapContainer) {
    ...
    const syncMapSplit = (panelWidth) => {
        if (!layoutContainer) {
            return;
        }
        const nextMargin = Math.max(0, Math.round(panelWidth + rightOffset + splitGap));
        layoutContainer.style.paddingRight = `${nextMargin}px`;
        refreshSplitMapLayout();
    };
    ...
    const clearMapSplit = () => {
        ...
        layoutContainer.style.paddingRight = '';
        refreshSplitMapLayout();
    };

    window.__modal3dSyncMapSplit = syncMapSplit;
    window.__modal3dClearMapSplit = clearMapSplit;
    ...
    const closeModal3d = () => {
        ...
        try {
            iframe.contentWindow.postMessage({ type: 'tar1090-3d-close' }, '*');
        } catch (e) { }
    ...
    window.__closeModal3dView = closeModal3d;

    const setDefaultModalSplit = () => {
        const maxWidth = Math.max(minWidth, window.innerWidth - minLeft - rightOffset);
        const targetWidth = Math.max(minWidth, Math.round(window.innerWidth * 0.5));
        const nextWidth = Math.min(maxWidth, targetWidth);
        modalContent.style.width = `${nextWidth}px`;
        modalContent.style.right = `${rightOffset}px`;
        modalContent.style.left = 'auto';
        return nextWidth;
    };

    window.__modal3dSetDefaultSplit = setDefaultModalSplit;

    const syncRenderedModalSplit = () => {
        const rect = modalContent.getBoundingClientRect();
        const width = rect.width || parseFloat(modalContent.style.width) || minWidth;
        syncMapSplit(width);
    };

    window.__modal3dSyncRenderedSplit = syncRenderedModalSplit;

    const onMouseMove = (event) => {
        if (!resizing) return;

        const maxLeftByWidth = fixedRightEdge - minWidth;
        const clampedLeft = Math.min(maxLeftByWidth, Math.max(minLeft, event.clientX));
        const nextWidth = fixedRightEdge - clampedLeft;

        modalContent.style.width = `${nextWidth}px`;
        modalContent.style.right = `${rightOffset}px`;
        modalContent.style.left = 'auto';
        syncMapSplit(nextWidth);
    };
    ...
    handle.addEventListener('mousedown', (event) => {
        ...
        resizing = true;
        const rect = modalContent.getBoundingClientRect();
        fixedRightEdge = rect.right;
        // Keep right side anchored to viewport edge while resizing.
        modalContent.style.right = `${rightOffset}px`;
        modalContent.style.left = 'auto';
        syncMapSplit(rect.width);
        if (modalIframe) {
    ...
    if (modalIframe) {
        modalIframe.addEventListener('load', () => {
            if (document.getElementById('iframeModal')?.style.display !== 'none') {
                syncRenderedModalSplit();
            }
        });
    }
...
function setSelectedIcao() {
    const selected = SelectedPlane;
    const map3dButton = document.getElementById('map3dButton');
    const expected3dUrlPrefix = '/3d.html?hex=' + selected.icao;
    const hasExpected3dUrl = map3dButton && map3dButton.dataset &&
        typeof map3dButton.dataset.url === 'string' &&
        map3dButton.dataset.url.indexOf(expected3dUrlPrefix) === 0;
    if (selected.icao == selIcao && copiedIcao == null && hasExpected3dUrl) {
        return;
    }
    selIcao = selected.icao;
    let hex_html = "<span style='font-family: monospace;'>Hex:" + NBSP + selected.icao.toUpperCase() +
        "</span>";
    jQuery('#selected_icao').html(hex_html);

    if (map3dButton) {
        map3dButton.dataset.url = '/3d.html?hex=' +
            selected.icao + '&debug=' + debugTracks + '&r=' + Math.random();
        map3dButton.style.display = 'inline-block';
    }

    jQuery('a.identSmall').prop('href', shareLink);
    initModal3dLeftResize();

    init3dModalControls();
}

let modal3dControlsInitialized = false;
function open3dModal(url) {
    ...
    if (window.__modal3dSetDefaultSplit) {
        window.__modal3dSetDefaultSplit();
    }
    if (window.__modal3dSyncRenderedSplit) {
        window.__modal3dSyncRenderedSplit();
    }
    requestAnimationFrame(() => {
        if (window.__modal3dSyncRenderedSplit) {
            window.__modal3dSyncRenderedSplit();
            setTimeout(window.__modal3dSyncRenderedSplit, 80);
        }
    });
}

function init3dModalControls() {
    if (modal3dControlsInitialized) {
        return;
    }
    modal3dControlsInitialized = true;

    const map3dButton = document.getElementById('map3dButton');
    if (map3dButton) {
        map3dButton.addEventListener('click', function (event) {
            event.stopPropagation();
            open3dModal(this.dataset.url);
        });
    }

    window.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && window.__closeModal3dView) {
            window.__closeModal3dView();
        }
    });
