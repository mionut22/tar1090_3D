    ...
    jQuery.ajax({
        ...
        .done((routes) => {
            ...
            for (let r in routes) {
                ...
                if (route.airport_codes == 'xxx-xxx') {
                    let tmpStr = "<span><b>" + route.airline + "</b><br>No route found</span>";
                    let tmpCoords = [0, 0];
                    g.route_cache[route.callsign] = [tmpStr, "No route", tmpCoords];
                } else if (route.airport_codes != 'unknown') {
                    let regionNames = new Intl.DisplayNames(['en'], { type: 'region' });
                    let tmpCoords = [];
                    let tmpStr = "<b>" + route.airline + "</b><br>";
                    for (let i = 0; i < route._airports.length; i++) {
                        tmpStr += route._airports[i].location + ", " +
                            regionNames.of(route._airports[i].countryiso2) +
                            "<br><sup>" +
                            route._airports[i].name.replace("International", "Int'l") +
                            " - <b>" + route._airports[i].iata +
                            "</b></sup>";
                        if (i < route._airports.length - 1)
                            tmpStr += "<br>";
                        if (i > 0 && route._airports.length > 0) {
                            tmpCoords.push([route._airports[i].lat, route._airports[i].lon]);
                        }
                    }
...
PlaneObject.prototype.setFlight = function (flight) {
    if (flight == null) {
        ...
        this.flightTs = now;
        let currentName = normalized_callsign(this.name);
        let plane_in_route = g.route_cache[currentName];
        if (plane_in_route === undefined) {
            this.routeCheck();
        } else {
            try {
                this.routeVerbose = g.route_cache[currentName][0];
                this.routeShort = g.route_cache[currentName][1];
                this.destCoords = g.route_cache[currentName][2];
            } catch (error) {
                console.error(error);
